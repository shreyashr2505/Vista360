import {
  ...
  registerButton,
  utils_exports
} from "./chunk-37X6ZH64.js";
import "./chunk-PR4QN5HX.js";

// node_modules/@photo-sphere-viewer/autorotate-plugin/index.module.js
...
var _AutorotateEvent = class _AutorotateEvent2 extends TypedEvent {
  /** @internal */
  constructor(autorotateEnabled) {
    super(_AutorotateEvent2.type);
...
var AutorotateButton = class extends AbstractButton {
  constructor(navbar) {
    ...
    (_a = this.plugin) == null ? void 0 : _a.addEventListener(AutorotateEvent.type, this);
  }
  destroy() {
    var _a;
    (_a = this.plugin) == null ? void 0 : _a.removeEventListener(AutorotateEvent.type, this);
    super.destroy();
  }
  isSupported() {
    return !!this.plugin;
...
var getConfig = utils_exports.getConfigParser(
  {
    ...
    autorotateSpeed: utils_exports.parseSpeed("2rpm"),
  ...
  },
  {
    autostartOnIdle: (autostartOnIdle, { rawConfig }) => {
      if (autostartOnIdle && utils_exports.isNil(rawConfig.autostartDelay)) {
        utils_exports.logWarn("autostartOnIdle requires a non null autostartDelay");
        return false;
      }
      return autostartOnIdle;
    },
    autorotateSpeed: (autorotateSpeed) => {
      return utils_exports.parseSpeed(autorotateSpeed);
    },
    autorotatePitch: (autorotatePitch) => {
      if (!utils_exports.isNil(autorotatePitch)) {
        return utils_exports.parseAngle(autorotatePitch, true);
      }
      return null;
    },
    autorotateZoomLvl: (autorotateZoomLvl) => {
      if (!utils_exports.isNil(autorotateZoomLvl)) {
...
var _AutorotatePlugin = class _AutorotatePlugin2 extends AbstractConfigurablePlugin {
  constructor(viewer, config) {
    ...
    this.state.initialStart = !utils_exports.isNil(this.config.autostartDelay);
  }
  static withConfig(config) {
    return [_AutorotatePlugin2, config];
  }
  /**
   * @internal
   */
  init() {
    ...
    this.viewer.addEventListener(events_exports.StopAllEvent.type, this);
    this.viewer.addEventListener(events_exports.BeforeRenderEvent.type, this);
    if (!this.video) {
      this.viewer.addEventListener(events_exports.KeypressEvent.type, this);
    }
  }
  /**
   * @internal
   */
  destroy() {
    this.viewer.removeEventListener(events_exports.StopAllEvent.type, this);
    this.viewer.removeEventListener(events_exports.BeforeRenderEvent.type, this);
    this.viewer.removeEventListener(events_exports.KeypressEvent.type, this);
    delete this.video;
    delete this.markers;
  ...
  handleEvent(e) {
    switch (e.type) {
      case events_exports.StopAllEvent.type:
        this.stop();
        break;
      case events_exports.BeforeRenderEvent.type: {
        this.__beforeRender(e.timestamp);
        break;
      }
      case events_exports.KeypressEvent.type:
        this.__onKeyPress(e);
        break;
    }
  }
  ...
  setKeypoints(keypoints) {
    if (!keypoints) {
      ...
      this.keypoints = keypoints.map((pt, i) => {
        ...
        } else if (utils_exports.isExtendedPosition(pt)) {
          position = pt;
        } else {
  ...
  __animate() {
    let p;
    if (!utils_exports.isNil(this.config.autorotateZoomLvl)) {
    ...
    p.then((done) => {
      if (done) {
        ...
        if (!utils_exports.isNil(this.config.autorotatePitch)) {
  ...
  __shiftKeypoints() {
    ...
    const index = this.__findMinIndex(this.keypoints, (keypoint) => {
      return utils_exports.greatArcDistance(keypoint.position, currentPosition);
    });
  ...
  __nextStep() {
    if (this.state.curve.length === 0) {
      this.__nextPoint();
      this.state.endStep[0] = utils_exports.parseAngle(this.state.endStep[0]);
    }
    this.state.startStep = this.state.endStep;
    this.state.endStep = this.state.curve.shift();
    const distance = utils_exports.greatArcDistance(this.state.startStep, this.state.endStep);
  ...
  __onKeyPress(e) {
    if (this.viewer.state.keyboardEnabled && e.matches(constants_exports.KEY_CODES.Space)) {
      this.toggle();
      e.preventDefault();
    }
  }
...
@photo-sphere-viewer/autorotate-plugin/index.module.js:
   ...
   *)
*/
//# sourceMappingURL=@photo-sphere-viewer_autorotate-plugin.js.map
