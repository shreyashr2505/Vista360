...
var _EquirectangularAdapter = class _EquirectangularAdapter extends AbstractAdapter {
  ...
  createMesh(panoData) {
    if (this.config.shader) {
      const geometry = new import_three4.SphereGeometry(SPHERE_RADIUS, 32, 16).scale(-1, 1, 1);
      const uniforms = {
        map: { value: null },
        opacity: { value: 1 },
        radius: { value: SPHERE_RADIUS },
        uvOffset: {
          value: new import_three4.Vector2(
...
var CONFIG_PARSERS = {
  ...
  fisheye: (fisheye) => {
    ...
    return import_three6.MathUtils.clamp(fisheye, 0, 2);
  },
