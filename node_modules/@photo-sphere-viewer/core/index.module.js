  ...
  if (adapter) {
    for (const [, p] of [["_", adapter], ...Object.entries(adapter)]) {
      if (p.prototype instanceof AbstractAdapter) {
        checkVersion(p.id, p.VERSION, "5.14.3");
        return p;
      }
    }
  }
...
_EquirectangularAdapter.VERSION = "5.14.3";
...
_DualFisheyeAdapter.VERSION = "5.14.3";
